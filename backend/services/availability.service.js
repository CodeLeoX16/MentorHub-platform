...
const getMentorAvailabilityForNext14Days = async (
  ...
  try {
    const slotDuration = Number(durationInMinutes) || 30;
    console.log("[availability] mentor", userId.toString(), "duration", slotDuration);
    ...
    const hasAnySlotConfigured = Object.values(weeklyAvailability || {}).some(
      (arr) => Array.isArray(arr) && arr.length > 0
    );

    if (!hasAnySlotConfigured) {
      throw new ApiError(
        httpStatus.badRequest,
        "No availability windows configured for this mentor."
      );
    }
    ...
    console.log(
      "[availability] generated days",
      next14DaysAvailability.length
    );
    ...
    throw new ApiError(
      httpStatus.badRequest,
      "Error retrieving mentor availability."
    );
  }
};
